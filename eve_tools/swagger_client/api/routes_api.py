...
class RoutesApi(object):
    ...
    def get_route_origin_destination_with_http_info(self, destination, origin, **kwargs):
        ...
        if self.api_client.client_side_validation and ('avoid' in params and
            ...
            raise ValueError(
                "Invalid value for parameter `avoid` when calling `get_route_origin_destination`, number of items must be less than or equal to `100`")
        if self.api_client.client_side_validation and ('connections' in params and
                                                       len(params['connections']) > 100):
            raise ValueError(
                "Invalid value for parameter `connections` when calling `get_route_origin_destination`, number of items must be less than or equal to `100`")
        collection_formats = {}
